...
const EMPTY_LEAD = { name: "", phone: "", email: "", eventType: "", eventDate: "", eventLocation: "", guestCount: "", budget: "", leadSource: "Website", source: "CRM", message: "", assignedTo: "", nextFollowUp: "" };
...
function mapRecord(row) {
  return {
    ...
    sourceDetail: SOURCE_DETAIL_ALIASES[String(row.source || "").toLowerCase()] || row.source || "Inquiry Form",
...
export default function AdminInquiries() {
  ...
  const [sourceDetails, setSourceDetails] = useState(DEFAULT_SOURCE_DETAILS);
  ...
  async function refresh() {
    try {
      ...
      setRecords(next);
      if (Array.isArray(result.sourceDetails)) setSourceDetails(Array.from(new Set([...DEFAULT_SOURCE_DETAILS, ...result.sourceDetails])));
      if (selected) {
  ...
  const sourceCounts = useMemo(() => sourceDetails.map((source) => ({ source, count: leads.filter((r) => r.sourceDetail === source).length })), [leads, sourceDetails]);
  const filtered = useMemo(() => {
    ...
    return base.filter((r) => {
      ...
      if (dateFrom && String(r.createdAt || "").slice(0, 10) < dateFrom) return false;
      if (dateTo && String(r.createdAt || "").slice(0, 10) > dateTo) return false;
      if (q && ![r.name, r.phone, r.email, r.eventType, r.eventLocation, r.id, r.leadSource, r.sourceDetail].some((v) => String(v || "").toLowerCase().includes(q))) return false;
      return true;
    });
  }, [tab, leads, bookings, stage, sourceFilter, eventFilter, assignedFilter, query, dateFrom, dateTo]);

  const pageSize = 10;
  const pageCount = Math.max(1, Math.ceil(filtered.length / pageSize));
  const pageRows = filtered.slice((page - 1) * pageSize, page * pageSize);
  useEffect(() => { setPage(1); }, [tab, stage, sourceFilter, eventFilter, assignedFilter, query, dateFrom, dateTo]);
  ...
  function resetFilters() { setStage("all"); setSourceFilter("all"); setEventFilter("all"); setAssignedFilter("all"); setQuery(""); setDateFrom(""); setDateTo(""); setPage(1); }
  function chooseDateRange() {
    ...
    if (to === null) return;
    if (from && !/^\\d{4}-\\d{2}-\\d{2}$/.test(from)) { setError("Use the date format YYYY-MM-DD."); return; }
    if (to && !/^\\d{4}-\\d{2}-\\d{2}$/.test(to)) { setError("Use the date format YYYY-MM-DD."); return; }
    if (from && to && from > to) { setError("Start date cannot be after end date."); return; }
    setDateFrom(from); setDateTo(to); setPage(1); setError("");
  }
  ...
  async function handleSaveSource(id) {
    try {
      await updateAdminInquiry(id, { source: sourceEdit });
      await refresh();
      setFeedback("Source updated and marked MANUAL.");
      setTimeout(() => setFeedback(""), 2500);
  ...
  async function handleScheduleFollowUp(record) {
    const value = window.prompt("Enter follow-up date and time (YYYY-MM-DDTHH:mm):", record.next_follow_up ? String(record.next_follow_up).slice(0, 16) : "");
    if (value === null) return;
    if (!value.trim()) return;
    try {
      await updateAdminInquiry(record.id, { nextFollowUp: new Date(value).toISOString() });
      await refresh();
      setFeedback("Follow-up scheduled.");
      setTimeout(() => setFeedback(""), 2500);
  ...
  function openEditLead(record) {
    ...
    setLeadForm({
      name: record.name === "Anonymous Client" ? "" : record.name,
      phone: record.phone || "",
      email: record.email || "",
      ...
      leadSource: record.leadSource || "Website",
      source: record.sourceDetail || "CRM",
  ...
  async function handleAddLead(e) {
    ...
    try {
      if (editingLead) {
        await updateAdminInquiry(editingLead.id, leadForm);
        await refresh();
        setShowAddLead(false);
        setEditingLead(null);
        setLeadForm(EMPTY_LEAD);
        setFeedback("Lead details updated.");
      } else {
  ...
  return (
    <div className="crm-page">
      <header className="crm-topbar">
        ...
        <button className="crm-add-button" type="button" onClick={() => { setEditingLead(null); setLeadForm(EMPTY_LEAD); setShowAddLead(true); }}><Icon name="plus" /> Add Lead</button>
      ...
      <div className="crm-filter-row">
        <button className="crm-filter-control crm-date-filter" type="button" onClick={chooseDateRange}><Icon name="calendar" /> {dateFrom || dateTo ? `${dateFrom || "Any"} – ${dateTo || "Any"}` : "All Created Dates"} <span>⌄</span></button>
        ...
        <label className="crm-filter-control"><span>Source</span><select value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value)}><option value="all">All Sources</option>{sourceDetails.map((source) => <option key={source} value={source}>{source}</option>)}</select></label>
          ...
          <div className="crm-table-wrap"><table className="crm-table"><thead><tr><th><input type="checkbox" aria-label="Select all" /></th><th>Customer</th><th>Event</th><th>Event Date</th><th>Location</th><th>Budget</th><th>Source</th><th>Status</th><th>Next Follow-up</th><th>Assigned To</th><th>Actions</th></tr></thead><tbody>
            {pageRows.map((r) => (
              <tr key={r.id} className={selected?.id === r.id ? "selected" : ""} onClick={() => selectRecord(r)}>
                ...
                <td>{r.budget || "—"}</td>
                <td><div className="crm-source-cell"><span className="crm-source-mini"><Icon name={SOURCE_ICONS[r.leadSource] || "layers"} /></span>{r.sourceDetail}</div><small className="crm-source-type">{r.sourceType}</small></td>
      ...
      {selected && (
        <aside className="crm-detail-drawer">
          ...
          <div className="crm-quick-actions"><button type="button" onClick={() => createQuotation(selected)}>▣ Create Quotation</button><button type="button" onClick={() => handleScheduleFollowUp(selected)}>▣ Schedule Follow-up</button><button type="button" onClick={() => handleStageChange(selected.id, "deal_closed")}>✓ Convert to Booking</button><button type="button" onClick={() => handleStageChange(selected.id, "lost")}>× Mark as Lost</button><button type="button" onClick={() => handleStageChange(selected.id, "junk")}>▢ Mark as Junk</button><button type="button" onClick={() => selected.phone && window.open(`https://wa.me/${String(selected.whatsapp || selected.phone).replace(/\\D/g, "")}`, "_blank", "noopener,noreferrer")}>➤ Send Message</button></div>
        </aside>
      )}
      ...
      {showAddLead && (
        <div className="crm-modal-backdrop" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) setShowAddLead(false); }}>
          <form className="crm-add-lead-modal" onSubmit={handleAddLead}>
            <div className="crm-modal-heading"><div><h2>{editingLead ? "Edit Lead" : "Add New Lead"}</h2><p>Capture enquiries from any source and convert them into beautiful events.</p></div><button type="button" onClick={() => { setShowAddLead(false); setEditingLead(null); }}><Icon name="close" /></button></div>
            ...
            <div className="crm-form-grid">
              ...
              <label>Source *<div><Icon name="send" /><input list="crm-source-details" value={leadForm.source} onChange={(e) => updateLeadField("source", e.target.value)} placeholder="e.g. Inquiry Form" required /></div></label>
              <label>Assign To<div><Icon name="user" /><select value={leadForm.assignedTo} onChange={(e) => updateLeadField("assignedTo", e.target.value)}><option value="">Auto assign</option>{teamMembers.map((member) => <option key={member}>{member}</option>)}</select></div></label>
            ...
            <datalist id="crm-source-details">{sourceDetails.map((source) => <option key={source} value={source} />)}</datalist><div className="crm-modal-actions"><button type="button" className="crm-cancel" onClick={() => { setShowAddLead(false); setEditingLead(null); setLeadForm(EMPTY_LEAD); }}>Cancel</button><button type="submit" className="crm-gold-button" disabled={savingLead}>{savingLead ? (editingLead ? "Saving…" : "Adding…") : (editingLead ? "Save Changes" : "Add Lead")}</button></div>
          </form>
        </div>
      )}
    </div>
