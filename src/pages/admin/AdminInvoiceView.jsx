...
const STATUS_LABEL = { draft: "Draft", sent: "Sent", partially_paid: "Partially paid", paid: "Paid", overdue: "Overdue", written_off: "Written off", cancelled: "Cancelled", converted: "Converted" };
...
export default function AdminInvoiceView() {
  ...
  usePageMeta(invoice?.documentType === "quotation" ? "Quotation — Admin" : "Invoice — Admin", "Admin panel.", { noindex: true });
  ...
  function convertToInvoice() {
    if (!isQuotation || invoice.convertedToInvoiceId) return;
    ...
    if (!converted) return;
    saveInvoice({ ...invoice, status: "converted", convertedToInvoiceId: converted.id });
    if (invoice.leadId) updateAdminInquiry(invoice.leadId, { invoiceId: converted.id, status: "deal_closed" }).catch(() => {});
    ...
    <div className="admin-page">
      <div className="admin-invoice-toolbar admin-print-hide">
        ...
        <div className="admin-row-actions">
          {isQuotation ? (invoice.convertedToInvoiceId ? <Link className="btn btn-line btn-sm" to={`/admin/invoices/${invoice.convertedToInvoiceId}`}>View Invoice</Link> : <button type="button" className="btn btn-primary btn-sm" onClick={convertToInvoice}>Convert to Invoice</button>) : getInvoiceBalance(invoice) > 0.01 && <><button type="button" className="btn btn-line btn-sm" onClick={() => { setPaymentAmount(String(getInvoiceBalance(invoice))); setPaymentOpen(true); }}>Record Payment</button><button type="button" className="btn btn-ghost btn-sm" onClick={() => setWriteOffOpen(true)}>Write Off</button></>}
      ...
      <div className="admin-invoice-sheet">
        ...
        {!isQuotation && (settings.upiId || settings.bankDetails) && (
        ...
        {!isQuotation && <div className="admin-invoice-balance">
        ...
        {!isQuotation && (invoice.payments || []).length > 0 && <div className="admin-invoice-payment-history">
        ...
        {!isQuotation && invoice.writeOff?.reason && <p className="admin-invoice-notes"><strong>Write-off:</strong> {invoice.writeOff.reason} ({invoice.writeOff.date})</p>}
