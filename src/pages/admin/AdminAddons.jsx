...
function emptyAddon(scope = "global") {
  return { label: "", subLabel: "", icon: "sparkle", image: "", categoryPath: [], scope, active: true };
}
...
export default function AdminAddons() {
  usePageMeta("Event Add-ons — Admin", "Manage add-on categories, add-on products, and featured add-on cards.");
  ...
  const filteredAddons = addons.filter((a) => scopeFilter === "all" || a.scope === scopeFilter);
  ...
  function scopeLabel(scope) {
    if (scope === "global") return "All Occasions (Global)";
    const occ = occasions.find((o) => o.slug === scope);
    return occ ? occ.label : scope;
  }

  function handleSaveAddon(e) {
    e.preventDefault();
    setError("");
    if (!editingAddon.label.trim()) return setError("Add-on card title is required.");
    if (!editingAddon.categoryPath?.length) return setError("Select an add-on category for this featured card.");
    try {
      saveAddon(editingAddon);
      refresh();
      setEditingAddon(null);
      flash(`Saved add-on card "${editingAddon.label}".`);
    } catch (err) { setError(err.message || "Failed to save add-on card."); }
  }

  function handleDeleteAddon(id, label) {
    if (!window.confirm(`Delete add-on card "${label}"? This only removes the featured card, not its products.`)) return;
    deleteAddon(id);
    refresh();
    flash(`Deleted add-on card "${label}".`);
  }
  ...
  function handleSaveCategory(e) {
    ...
    if (!cat?.label?.trim()) return setError("Add-on category name is required.");
    try {
      saveCategory("event-add-ons", { ...cat, slug: sanitizeSlug(cat.slug || cat.label), parentCategorySlug: editingCategory.parentCategorySlug || null });
      refresh();
      setEditingCategory(null);
      flash(`Saved add-on category "${cat.label}".`);
    } catch (err) { setError(err.message || "Failed to save add-on category."); }
  }
  ...
  function handleDeleteCategory(trail) {
    ...
    if (productCount > 0) {
      setError(`Cannot delete "${node.label}" because it contains ${productCount} add-on product${productCount === 1 ? "" : "s"}. Move or delete those products first.`);
      return;
    }
    const parent = trail.length > 1 ? trail[trail.length - 2] : null;
    if (!window.confirm(`Delete add-on category "${node.label}" and its empty child categories?`)) return;
    deleteCategory("event-add-ons", node.slug, parent?.slug || null);
    refresh();
    flash(`Deleted add-on category "${node.label}".`);
  }
        ...
        <div key={key} className="admin-nested-node" style={{ marginLeft: depth * 22 }}>
          <div className="admin-subcategory-item">
            ...
            <div className="admin-row-actions">
              ...
              <a href={`/occasion/event-add-ons/${trail.map((n) => n.slug).join("/")}`} target="_blank" rel="noreferrer" className="btn-icon" title="View category"><Icon name="eye" /></a>
  ...
  return (
    <div className="admin-page">
      <div className="admin-page-head">
        <div>
          <h1>Event Add-ons</h1>
          <p className="admin-hint">Everything related to add-ons is managed here. <strong>Add-on Categories</strong> define the catalog structure, <strong>Add-on Products</strong> are the sellable services inside those categories, and <strong>Featured Cards</strong> controls which categories appear in the Popular Add-ons strip.</p>
        </div>
        <div className="admin-head-actions">
          ...
          {tab === "products" && <Link to="/admin/addons/products/new" className="btn btn-primary"><Icon name="plus" /> Add Add-on Product</Link>}
      ...
      <div className="admin-tabs">
        <button type="button" className={`admin-tab ${tab === "products" ? "active" : ""}`} onClick={() => setTab("products")}><Icon name="package" /> Add-on Products ({addonProducts.length})</button>
        <button type="button" className={`admin-tab ${tab === "categories" ? "active" : ""}`} onClick={() => setTab("categories")}><Icon name="grid" /> Add-on Categories ({categoryOptions.length})</button>
      ...
      {tab === "products" && (
        <div className="admin-panel">
          <div className="admin-toolbar-row">
            <div className="admin-search-wrapper"><Icon name="search" /><input type="search" className="admin-search" placeholder="Search add-on products..." value={productSearch} onChange={(e) => setProductSearch(e.target.value)} /></div>
            <div className="admin-filters">
              <select className="admin-select" value={productCategory} onChange={(e) => setProductCategory(e.target.value)}><option value="all">All Add-on Categories</option>{categoryOptions.map((c) => <option key={c.path.join("/")} value={c.path.join("/")}>{c.label}</option>)}</select>
          ...
          {filteredProducts.length === 0 ? <div className="admin-empty"><p>No add-on products match your filters.</p><Link to="/admin/addons/products/new" className="btn btn-sm btn-primary" style={{ marginTop: 12 }}>Create Add-on Product</Link></div> : (
            <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th style={{ width: 70 }}>Picture</th><th>Product</th><th>Category</th><th>Price</th><th>Status</th><th style={{ textAlign: "right" }}>Actions</th></tr></thead><tbody>
              {filteredProducts.map((product) => <tr key={product.id || product.slug}>
                ...
                <td><div className="admin-table-main-title"><Link to={`/admin/addons/products/${product.id || product.slug}/edit`}>{product.name}</Link>{product.badge && <span className="admin-badge-mini">{product.badge}</span>}</div><div className="admin-table-sub"><code>{product.slug}</code></div></td>
                ...
                <td style={{ textAlign: "right" }}><div className="admin-row-actions" style={{ justifyContent: "flex-end" }}><Link className="btn-icon" title="Edit add-on product" to={`/admin/addons/products/${product.id || product.slug}/edit`}><Icon name="edit" /></Link><a className="btn-icon" title="View" href={`/occasion/event-add-ons/${(product.categoryPath || []).slice(1).join("/")}/${product.slug}`} target="_blank" rel="noreferrer"><Icon name="eye" /></a></div></td>
              </tr>)}
            </tbody></table></div>
          )}
        </div>
      ...
      {tab === "categories" && (
        <div className="admin-panel">
          <div className="admin-calc-box"><div className="admin-calc-item"><span>Category structure</span><strong>Event Add-ons</strong></div><div className="admin-calc-item"><span>Categories / subcategories</span><strong>{categoryOptions.length}</strong></div><div className="admin-calc-item"><span>Sellable add-on products</span><strong>{addonProducts.length}</strong></div></div>
